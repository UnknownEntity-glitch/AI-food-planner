# inventory.py - Модуль учёта домашних запасов
...
# ------------------------------------------------------------
# Базовый словарь канонических названий
# ------------------------------------------------------------
SEED_CANONICAL = [
    "курица", "куриная грудка", "куриное филе", "лук", "чеснок",
    "масло растительное", "масло оливковое", "рис", "гречка", "овсянка", "сыр",
    "молоко", "яйцо", "авокадо", "тунец", "картофель",
    "морковь", "помидор", "огурец", "сахар", "соль", "перец чёрный молотый",
    "мука пшеничная", "мука", "мука рисовая", "мука кукурузная", "мука миндальная", "мука кокосовая",
    "йогурт", "кефир", "бекон", "паста", "макароны",
    "банан", "яблоко", "лимон", "капуста", "свёкла", "петрушка", "укроп",
    "сметана", "творог", "сливки", "говядина", "свинина", "баранина", "индейка",
    "рыба", "хлеб", "батон", "вода", "сода", "уксус", "корица", "майонез",
    "кетчуп", "горчица", "томатная паста", "соевый соус", "устричный соус",
    "сливочное масло", "пахта", "ряженка", "кабачок", "баклажан", "тыква",
    "редис", "сельдерей", "шпинат", "салат", "руккола", "зеленый лук",
    "брокколи", "цветная капуста", "спаржа", "шампиньоны", "белые грибы",
    "лисички", "вешенки", "имбирь", "куркума", "паприка", "тмин", "гвоздика",
    "мускатный орех", "лаваш", "тортилья", "миндаль", "грецкий орех", "фундук",
    "кешью", "арахис", "семена кунжута", "семена льна", "семена чиа",
    "изюм", "курага", "чернослив", "финики", "гречневая крупа", "овсяные хлопья",
    "овсяная мука", "булгур", "киноа", "перловка", "манка", "пшено",
    "ячневая крупа", "шоколад", "какао-порошок", "ванилин", "разрыхлитель",
    "желатин", "агар-агар", "кукурузный крахмал", "мёд", "клубника", "малина",
    "черника", "смородина", "вишня", "черешня", "персик", "абрикос", "груша",
    "ананас", "манго", "апельсин", "мандарин", "грейпфрут", "лайм", "виноград",
    "лосось", "семга", "форель", "треска", "кета", "креветки", "кальмары",
    "мидии", "устрицы", "телятина", "утка", "кролик", "сосиски", "сардельки",
    "колбаса", "ветчина", "грудинка", "окорок", "карбонат", "фарш", "нут",
    "фасоль", "чечевица", "горошек", "кукуруза", "тофу", "печень", "сердце",
    "почки", "язык", "сало", "сухари", "семечки", "орехи"
]

# ------------------------------------------------------------
# Синонимы единиц измерения
# ------------------------------------------------------------
UNIT_SYNONYMS = {
...
DEFAULT_QUANTITY_WORDS = {
    'зубчик': ('clove', 1), 'зубчика': ('clove', 1), 'зубчиков': ('clove', 1),
    'банка': ('can', 1), 'банки': ('can', 1), 'пачка': ('pack', 1), 'пакетик': ('pack', 1),
    'пучок': ('bunch', 1), 'пучка': ('bunch', 1), 'головка': ('head', 1), 'головки': ('head', 1),
    'кочан': ('head', 1), 'кочана': ('head', 1), 'щепотка': ('pinch', 1), 'щепотки': ('pinch', 1),
    'буханка': ('pc', 1), 'батон': ('pc', 1),
}

# ------------------------------------------------------------
# Вспомогательные функции нормализации и парсинга
# ------------------------------------------------------------
...
def canonicalize(raw: str, vocab: Optional[Iterable[str]] = None) -> str:
    """Приводит сырое название к каноническому виду."""
    if raw is None:
        return ''
    ...
    best_score = 0
    for t in tokens:
        cand = get_close_matches(t, list(vocab_norm.keys()), n=3, cutoff=0.75)
        if cand:
            score = len(t)
            if score > best_score:
                best_score = score
    ...
    cand_full = get_close_matches(n, list(vocab_norm.keys()), n=1, cutoff=0.75)
    if cand_full:
...
# ------------------------------------------------------------
# Класс для работы с БД (PantryManager)
# ------------------------------------------------------------
class PantryManager:
    ...
    def suggest_known_product(self, raw_name: str) -> Optional[str]:
        known = self.db.get_all_ingredients() + SEED_CANONICAL
        known = list(dict.fromkeys(known))
        matches = get_close_matches(raw_name, known, n=1, cutoff=0.75)
